...
Whenever a resolution leaves a shortfall (`paidAmount` below the cycle's
committed amount — via Skip or a partial Partial payment), `resolveCycle`
never assumes what it means: the caller must pass `shortfallOutcome`,
`'deferred'` (a Balance Due is spun off, due-dated at the user's next
paycheck — or today, as a fallback, if no paycheck is configured) or
`'forgiven'` (no Balance Due at all — nothing is owed). Choosing forgiven can
also pause the template (`pauseTemplateOnForgive`), folded into the same
atomic transaction so a failure can never leave a forgiven cycle with the
template still silently active. `shortfallOutcome`/`deferredDueDate`/
`templatePausedOnForgive` are absent (not merely falsy) on log rows written
before this existed, so display logic (`resolutionTypeOf` in
`FixedExpenses.jsx`) treats an absent `shortfallOutcome` the same as
`'deferred'` — no data migration was needed.
...
| `adjustmentExpenseId` | string \| null | Yes | The Balance Due row spun off for a deferred shortfall, or null when the cycle was paid in full or the shortfall was forgiven |
...
| `shortfallOutcome` | `'deferred'` \| `'forgiven'` \| null | No | What a shortfall means; null when there was no shortfall (Full, or a Partial capped at the full amount) |
| `deferredDueDate` | string \| null | No | The Balance Due's actual due date, set only when `shortfallOutcome === 'deferred'` — denormalized here so display code doesn't need a second fetch |
| `templatePausedOnForgive` | boolean | No | True only when the user chose to also pause the template while forgiving |
...
`getTemplatesDueForGeneration` and `autoGenerateDueExpenses`, which predated this file's other callers being switched to `dbHelpers.materializeDueTemplates()` directly, have been removed — they had zero callers anywhere in the app.
...
| `resolveCycle(id, { paidAmount, shortfallOutcome, pauseTemplateOnForgive })` | Pay Full / Partial / Skip a recurring template's current cycle via `dbHelpers.resolveCycle` — advances the template's cadence immediately. Any shortfall requires `shortfallOutcome` (`'deferred'` spins off a Balance Due due at the next paycheck; `'forgiven'` spins off nothing); `pauseTemplateOnForgive` additionally pauses the template, only legal alongside `'forgiven'`. Used by `ResolveExpenseModal`, the unified "mark paid" UI |
...
| `PriorityExpenseList` | `src/components/PriorityExpenseList.jsx` | Buckets every unpaid/partially-paid expense into Overdue/This week/Later via `getPaymentProgress()`/`getTimingBucket()`. Reads only materialized `fixedExpenses` rows — unlike the Calendar, it does **not** fall back to `virtualLedger.js`'s forecasted cycles, so a recurring bill's second-and-later occurrences are invisible here until `materializeDueTemplates` actually materializes them (see `allowFuture` note above) |
