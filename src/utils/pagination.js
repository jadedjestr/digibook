 ...
 */
export const createPaginatedQuery = async (queryFunction, page = 1, pageSize = PAGINATION_CONFIG.DEFAULT_PAGE_SIZE, ...args) => {
  try {
    start();
 ...
 */
export const createInfiniteScrollQuery = async (queryFunction, lastId = null, pageSize = PAGINATION_CONFIG.DEFAULT_PAGE_SIZE, ...args) => {
  try {
    start();
    
    const data = await queryFunction('infinite', { lastId, limit: pageSize }, ...args);
 ...
 */
export const createSearchQuery = async (searchFunction, query, page = 1, pageSize = PAGINATION_CONFIG.DEFAULT_PAGE_SIZE, ...args) => {
  try {
    start();
    ...
    const data = await searchFunction('data', query, { offset, limit }, ...args);
...
export const processBatch = async (items, batchSize = 100, processFunction) => {
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    try {
      ...
      // Log progress
      logger.debug(`Processed batch ${i + 1}/${totalBatches} (${batch.length} items)`);
    } catch (error) {
 ...
 */
export const streamData = async function* (queryFunction, pageSize = PAGINATION_CONFIG.DEFAULT_PAGE_SIZE, ...args) {
  let page = 1;
  let hasMore = true;
  
  while (hasMore) {
    try {
      const result = await createPaginatedQuery(queryFunction, page, pageSize, ...args);
...
const end = (operation) => {
  ...
  if (duration > 100) { // Log if operation takes more than 100ms
    logger.warn(`Slow pagination operation: ${operation} took ${duration.toFixed(2)}ms`);
  }
};
