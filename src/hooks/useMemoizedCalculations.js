 ...
 */
export const useMemoizedCalculations = (expenses, accounts, creditCards, paycheckSettings) => {
  ...
  const totals = useMemo(() => {
    ...
    const totalPaid = expenses.reduce((sum, expense) => sum + (expense.paidAmount || 0), 0);
  ...
  const expenseStatuses = useMemo(() => {
    ...
    const statuses = expenses.map(expense => {
      const remaining = expense.amount - (expense.paidAmount || 0);
      const isOverdue = expense.dueDate && new Date(expense.dueDate) < new Date();
      ...
      return {
        ...
        isOverdue,
        remaining,
        percentage: expense.amount > 0 ? ((expense.paidAmount || 0) / expense.amount) * 100 : 0,
      };
    });
  ...
  const getFilteredExpenses = useCallback((filters = {}) => {
    start();
      ...
      filtered = filtered.filter(expense => {
        ...
        switch (filters.status) {
          ...
          case 'unpaid':
            return expense.status !== 'paid' && remaining > 0;
          case 'overdue':
            return expense.dueDate && new Date(expense.dueDate) < new Date() && remaining > 0;
          default:
            return true;
        }
      });
