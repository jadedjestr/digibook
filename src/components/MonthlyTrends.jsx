    ...
    return (
      <div className="glass-panel">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-white/50 text-sm mt-2">Start making payments to see trends over time.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="glass-panel">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="glass-card p-4">
            ...
            <p className={`text-lg font-bold ${
              trends.budgetTrend > 0 ? 'text-red-400' : 
              trends.budgetTrend < 0 ? 'text-green-400' : 'text-white'
            }`}>
              {trends.budgetTrend > 0 ? '+' : ''}<PrivacyWrapper>{formatCurrency(trends.budgetTrend)}</PrivacyWrapper>
            </p>
          </div>
          ...
          <div className="glass-card p-4">
            ...
            <p className={`text-lg font-bold ${
              trends.actualTrend > 0 ? 'text-red-400' : 
              trends.actualTrend < 0 ? 'text-green-400' : 'text-white'
            }`}>
              {trends.actualTrend > 0 ? '+' : ''}<PrivacyWrapper>{formatCurrency(trends.actualTrend)}</PrivacyWrapper>
            </p>
          </div>
          ...
          <div className="glass-card p-4">
            ...
            <p className={`text-lg font-bold ${
              trends.overpaymentTrend > 0 ? 'text-orange-400' : 
              trends.overpaymentTrend < 0 ? 'text-green-400' : 'text-white'
            }`}>
              {trends.overpaymentTrend > 0 ? '+' : ''}<PrivacyWrapper>{formatCurrency(trends.overpaymentTrend)}</PrivacyWrapper>
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        {monthlyData.map((data, index) => {
          const budgetWidth = maxValue > 0 ? (data.totalBudget / maxValue) * 100 : 0;
          const actualWidth = maxValue > 0 ? (data.totalActual / maxValue) * 100 : 0;
          const overpaymentWidth = maxValue > 0 ? (data.totalOverpayment / maxValue) * 100 : 0;
          const accuracy = data.totalBudget > 0 ? (data.totalActual / data.totalBudget) * 100 : 0;
            ...
            <div key={`${data.year}-${data.month}`} className="glass-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-xs font-medium ${getAccuracyColor(data.totalActual, data.totalBudget)}`}>
              ...
              {data.totalOverpayment > 0 && (
                <div className="mt-2 text-xs text-orange-400">
                  Overpaid by <PrivacyWrapper>{formatCurrency(data.totalOverpayment)}</PrivacyWrapper>
                </div>
              )}
            </div>
          );
      ...
      <div className="mt-4 flex justify-between text-xs text-white/50">
        <span>$0</span>
        <span><PrivacyWrapper>{formatCurrency(maxValue)}</PrivacyWrapper></span>
      </div>
    </div>
  );
};
