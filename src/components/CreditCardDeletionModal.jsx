...
const CreditCardDeletionModal = ({
  ...
  const getUnassignedCount = () => {
    return Object.values(reassignmentMap).filter(accountId => !accountId)
      .length;
  };
              ...
              <div className='space-y-3'>
                <label className='flex items-start space-x-3 cursor-pointer'>
                  ...
                  <div>
                    <div className='text-white font-medium'>
                      Reassign to Other Accounts
                    </div>
                    <div className='text-white/60 text-sm'>
                      Move all linked expenses to different accounts
                      (recommended)
                    </div>
                  </div>
                </label>
                ...
                <label className='flex items-start space-x-3 cursor-pointer'>
                  ...
                  <div>
                    ...
                    <div className='text-white/60 text-sm'>
                      Delete credit card and leave expenses with invalid account
              ...
              <div className='text-sm text-white/70'>
                {getUnassignedCount() === 0 ? (
                  <span className='text-green-400'>
                    ✅ All expenses will be reassigned
                  </span>
                ) : (
                  <span className='text-orange-400'>
                    ⚠️ {getUnassignedCount()} expense
                    {getUnassignedCount() !== 1 ? 's' : ''} still need
                    {getUnassignedCount() === 1 ? 's' : ''} reassignment
                  </span>
                )}
              </div>
            </div>
