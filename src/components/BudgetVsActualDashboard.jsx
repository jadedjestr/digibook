...
const BudgetVsActualDashboard = ({ summary }) => {
  ...
  const getAccuracyColor = accuracy => {
    if (!accuracy || accuracy <= 100) return 'text-green-400';
    if (accuracy <= 120) return 'text-yellow-400';
    return 'text-red-400';
  };

  const getAccuracyIcon = accuracy => {
    if (!accuracy || accuracy <= 100) return CheckCircle;
    ...
    <div className='glass-panel'>
      <div className='flex items-center justify-between mb-6'>
        ...
        <div className='flex items-center space-x-2'>
          <AccuracyIcon
            className={`w-5 h-5 ${getAccuracyColor(budgetAccuracy)}`}
          />
          <span className={`font-semibold ${getAccuracyColor(budgetAccuracy)}`}>
            {(budgetAccuracy || 0).toFixed(1)}% of Budget
          </span>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
        ...
        <div className='glass-card p-4'>
          <div className='flex items-center justify-between mb-2'>
            ...
            <AlertTriangle
              className={`w-4 h-4 ${totalOverpayment > 0 ? 'text-orange-400' : 'text-green-400'}`}
            />
          </div>
          <PrivacyWrapper>
            <p
              className={`text-xl font-bold ${totalOverpayment > 0 ? 'text-orange-400' : 'text-green-400'}`}
            >
              {totalOverpayment > 0 ? '+' : ''}
        ...
        <div className='glass-card p-4'>
          <div className='flex items-center justify-between mb-2'>
            ...
            <CheckCircle
              className={`w-4 h-4 ${significantOverpayments > 0 ? 'text-orange-400' : 'text-green-400'}`}
            />
          </div>
          <p
            className={`text-xl font-bold ${significantOverpayments > 0 ? 'text-orange-400' : 'text-green-400'}`}
          >
            {significantOverpayments}
          </p>
          <p className='text-xs text-white/50 mt-1'>
            expenses &gt;20% over budget
          </p>
        </div>
      </div>
          ...
          <div className='flex items-start space-x-3'>
            ...
            <div>
              ...
              <p className='text-white/70 text-sm mt-1'>
                You&apos;ve overpaid by{' '}
                <PrivacyWrapper>
                  <strong>{formatCurrency(totalOverpayment)}</strong>
                </PrivacyWrapper>{' '}
                this cycle.
                {significantOverpayments > 0 && (
                  <>
                    {' '}
                    {significantOverpayments} expense
                    {significantOverpayments !== 1 ? 's' : ''} had significant
                    overpayments (&gt;20%).
                  </>
                )}
              </p>
              {(budgetAccuracy || 0) > 120 && (
                <p className='text-white/70 text-sm mt-1'>
                  Consider reviewing your budget allocations for better
                  accuracy.
                </p>
              )}
            </div>
