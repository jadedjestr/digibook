...
const DebtPayoffCalculator = ({ creditCards = [], onDataChange }) => {
  ...
  const calculatePayoff = async () => {
    ...
    try {
      const result = await dbHelpers.calculateDebtPayoff(balance, payment, interestRate);
  ...
  const getPayoffColor = () => {
    if (!payoffResult || payoffResult.payoffMonths === -1) return 'text-red-400';
  ...
  const creditUtilization = calculatorData.creditLimit > 0 
  ...
  if (creditCards.length === 0) {
    return (
      <div className="glass-panel">
        <h2 className="text-xl font-bold text-white mb-4">Debt Payoff Calculator</h2>
        <div className="text-center py-8">
          <CreditCard className="w-12 h-12 text-white/30 mx-auto mb-4" />
          <p className="text-white/70">No credit cards found.</p>
          <p className="text-white/50 text-sm mt-2">Add credit cards to use the payoff calculator.</p>
        </div>
      </div>
    );
  }
            ...
            <div className="glass-card p-4">
              ...
              {isEditing ? (
                <input
                  ...
                  onBlur={() => updateCreditCard({ balance: calculatorData.balance })}
                  className="glass-input w-full"
                  step="0.01"
                  min="0"
                />
            ...
            <div className="glass-card p-4">
              ...
              {isEditing ? (
                <input
                  ...
                  onBlur={() => updateCreditCard({ creditLimit: calculatorData.creditLimit })}
                  className="glass-input w-full"
                  step="0.01"
                  min="0"
                />
              ...
              <p className={`text-sm mt-1 ${getUtilizationColor(creditUtilization)}`}>
            ...
            <div className="glass-card p-4">
              ...
              {isEditing ? (
                <input
                  ...
                  onBlur={() => updateCreditCard({ interestRate: calculatorData.interestRate })}
                  className="glass-input w-full"
                  step="0.01"
                  min="0"
                  max="50"
          ...
          {payoffResult && (
            <div className="space-y-4">
              ...
              {payoffResult.payoffMonths === -1 ? (
                <div className="glass-card p-4 border border-red-500/20 bg-red-500/10">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-red-400 font-medium">Payment Too Low</p>
                      <p className="text-white/70 text-sm">
                        Monthly payment of {formatCurrency(calculatorData.payment)} doesn't cover 
                        interest ({formatCurrency(payoffResult.monthlyInterest)}/month). 
                  ...
                  <div className="glass-card p-4">
                    ...
                    <PrivacyWrapper>
                      <p className="text-xl font-bold text-purple-400">
                        {formatCurrency(calculatorData.balance + payoffResult.totalInterest)}
                      </p>
                    </PrivacyWrapper>
                  </div>
                </div>
              ...
              {payoffResult.payoffMonths > 0 && (
                <div className="glass-card p-4">
                  <h4 className="font-medium text-white mb-3">What if you paid more?</h4>
                  <div className="space-y-2 text-sm">
                    {[50, 100, 200].map(extra => {
                      ...
                      const interestSaved = payoffResult.totalInterest - totalInterest;

                      return (
                        <div key={extra} className="flex justify-between items-center py-2 border-b border-white/10 last:border-b-0">
                          <span className="text-white/70">
                            +{formatCurrency(extra)}/month ({formatCurrency(newPayment)} total)
                          </span>
                          <span className="text-green-400">
                            Save {timeSaved} months, <PrivacyWrapper>{formatCurrency(interestSaved)}</PrivacyWrapper> interest
                          </span>
                        </div>
                      );
                    })}
