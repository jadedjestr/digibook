...
const CreditCardMigrationModal = ({ isOpen, onClose, onComplete }) => {
  ...
  const applyMappings = async () => {
    ...
    try {
      ...
      // Clean up duplicates
      const duplicatesRemoved = await dbHelpers.cleanupDuplicateCreditCardExpenses();
      
      // Create missing expenses for credit cards that still don't have any linked expenses
      const missingExpensesCreated = await dbHelpers.createMissingCreditCardExpenses();
      ...
      const totalActions = results.appliedCount + duplicatesRemoved.length + missingExpensesCreated;
      notify.success(`Migration complete! Mapped ${results.appliedCount} expenses, removed ${duplicatesRemoved.length} duplicates, and created ${missingExpensesCreated} missing expenses`);
      ...
      <div className="bg-gray-800/90 backdrop-blur-md rounded-xl shadow-2xl border border-white/20 w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-between items-center p-6 border-b border-white/10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-white">Credit Card Migration</h2>
              <p className="text-sm text-gray-400">Link existing expenses to credit card accounts</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 p-6 overflow-y-auto min-h-0">
            ...
            <div className="space-y-4">
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="font-medium text-white mb-1">Detection Results</h3>
                    <p className="text-sm text-gray-300">
                      Found {detectedMappings.length} potential credit card expense mappings. 
              ...
              {detectedMappings.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-400">No potential credit card mappings found.</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="text-white font-medium">{mapping.expenseName}</div>
                            <div className="text-sm text-gray-400">
                              Map to: <span className="text-blue-300">{mapping.creditCardName}</span>
                            </div>
                          </div>
                        </div>
          ...
          {step === 'apply' && (
            <div className="text-center py-12">
              ...
              <p className="text-white">Applying mappings, cleaning up duplicates, and creating missing expenses...</p>
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="font-medium text-white mb-1">Migration Complete!</h3>
                    <p className="text-sm text-gray-300">
                      Successfully mapped {migrationResults.appliedCount} expenses, removed {migrationResults.duplicatesRemoved.length} duplicates, and created {migrationResults.missingExpensesCreated || 0} missing expenses.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {migrationResults.missingExpensesCreated > 0 && (
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-white mb-1">Missing Expenses Created</h4>
                      <p className="text-sm text-gray-300">
                        Created {migrationResults.missingExpensesCreated} new fixed expenses for credit cards that didn't have any.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {migrationResults.results.map((result, index) => (
                    <div key={index} className="bg-white/5 rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-white text-sm">{result.expenseName}</div>
                          <div className="text-gray-400 text-xs">→ {result.creditCardName}</div>
                        </div>
        ...
        <div className="flex justify-between items-center p-6 border-t border-white/10 bg-gray-800/95 backdrop-blur-md">
          <div className="text-sm text-gray-400">
            {step === 'review' && `${selectedMappings.length} of ${detectedMappings.length} selected`}
          </div>
          <div className="flex space-x-3">
            ...
            {step === 'review' && (
              <button
                ...
                disabled={selectedMappings.length === 0 || isLoading}
